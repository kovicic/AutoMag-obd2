...
static void _event_handler(telnet_t *telnet, telnet_event_t *ev,
	...
	case TELNET_EV_DATA:
		...
		//telnetCall((char*)ev->data.buffer, ev->data.size);
		fflush(stdout);
		break;
...
int8_t TEL_registerCallback(telnetCallback callbackTelnetFunction)
{
	if(callbackTelnetFunction == NULL)
	{
		printf("Invalid callback function\n");
		return TEL_ERROR;
	}
	telnetCall = callbackTelnetFunction;
	return TEL_SUCCESS;

}
int8_t TEL_deRegisterCallback()
{
	telnetCall = NULL;
	return TEL_SUCCESS;
}
